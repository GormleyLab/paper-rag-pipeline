...
class MetadataExtractor:
    ...
    def __init__(
        ...
        if crossref_email:
            self.session.headers.update(
                {"User-Agent": f"PaperRAG/1.0 (mailto:{crossref_email})"}
            )
    ...
    def extract_metadata(
        ...
        logger.warning(
            f"Using document parsing for {pdf_path.name} (may be incomplete)"
        )
        return self._parse_metadata_from_text(
            first_pages_text or "", pdf_path, existing_keys
        )

    def _get_metadata_from_pdf2bib(
        self, pdf_path: Path, existing_keys: set[str]
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                if "identifier" in result:
                    ...
                    logger.warning(
                        f"pdf2bib returned unexpected result format for {pdf_path.name}"
                    )
                    return None
            else:
            ...
            if not bibtex_entry:
                logger.warning(
                    f"pdf2bib found identifier but no BibTeX data for {pdf_path.name}"
                )
                return None
            ...
            if not parsed:
                logger.warning(
                    f"Failed to parse BibTeX from pdf2bib for {pdf_path.name}"
                )
                return None

            # Generate proper BibTeX key
            bibtex_key = generate_bibtex_key(
                parsed["authors"], parsed["year"], existing_keys
            )
            ...
            doi = parsed.get("doi") or (
                identifier if identifier_type == "doi" else None
            )
    ...
    def _get_metadata_from_crossref(
        self, doi: str, existing_keys: set[str]
    ) -> Optional[PaperMetadata]:
        """
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if parsed:
                        ...
                        # Replace the key in the BibTeX entry
                        bibtex_entry = self._replace_bibtex_key(
                ...
                logger.warning(
                    f"CrossRef API request failed (attempt {attempt + 1}): {e}"
                )
    ...
    def _get_metadata_from_arxiv(
        self, arxiv_id: str, existing_keys: set[str]
    ) -> Optional[PaperMetadata]:
        """
    ...
    def _get_metadata_from_pubmed(
        self, pmid: str, existing_keys: set[str]
    ) -> Optional[PaperMetadata]:
        """
        ...
        logger.info(
            f"PubMed API integration not fully implemented yet for PMID: {pmid}"
        )
        return None
    ...
    def _parse_metadata_from_text(
        self, text: str, pdf_path: Path, existing_keys: set[str]
    ) -> PaperMetadata:
        """
